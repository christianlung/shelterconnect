  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
      {shelters.map((shelter, index) => {
        // Combine accommodations and true boolean flags into one array.
        const accommodationTags = [
          ...shelter.accommodations,
          ...(shelter.wheelchairAccessible ? ["Wheelchair Accessible"] : []),
          ...(shelter.housesLargeAnimals ? ["Houses Large Animals"] : []),
          ...(shelter.housesSmallAnimals ? ["Houses Small Animals"] : []),
          ...(shelter.hasCounselingUnit ? ["Counseling Unit"] : []),
          ...(shelter.foodProvided ? ["Food Provided"] : []),
          ...(shelter.waterProvided ? ["Water Provided"] : []),
        ];
                  ...
                  <div>
                    ...
                    <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, mb: 1 }}>
                      {accommodationTags.map((tag, idx) => (
                        <Chip key={idx} label={`#${tag}`} sx={{ bgcolor: "#b3e5fc", color: "#01579b" }} />
                      ))}
                    </Box>
                  </div>
                )}
